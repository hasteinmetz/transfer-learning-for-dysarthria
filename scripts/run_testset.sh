...
echo "Done all experiments! Tagging and pushing to Github"
DATE=$(date '+%D-%H:%M')
EXPTNAMES="${EXPERIMENTS[*]// /|}"
git add results && git commit -m "($DATE) test set: $EXPTNAMES"
git tag -a "results-$DATE" -m "($DATE) test set: $EXPTNAMES"
git push
